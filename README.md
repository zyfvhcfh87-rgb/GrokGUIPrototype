...
`npm run tauri build -- --bundles nsis` produces the Windows installer. `npm run tauri build -- --no-bundle` verifies the production desktop build without producing an installer. Keep the existing Rust workspace green with `cargo check --workspace --all-targets` and `cargo test --workspace --all-targets`.
