...
- The Rust crate registers only setup/workspace, GUI presentation preferences, lifecycle, session, turn/control, interaction-response, read-only Git inspection, and bounded runtime-diagnostics operations; it initializes no plugins. The generic runtime command enum is not a Tauri input. The bridge accepts no raw JSON-RPC payload, executable path, launch argument, credential, native handle, or unrestricted filesystem operation. Git is spawned with an argument array and never through a shell.
...
- Presentation preferences (theme, motion, panel layout, onboarding) live in a separate versioned file in the app config directory. They never include workspace contents, session IDs, or Grok credentials. Application exit stops the contained GrokRuntime child so the GUI does not leave an orphaned ACP process.
