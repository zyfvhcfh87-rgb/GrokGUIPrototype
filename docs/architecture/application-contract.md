...
Each command accepts one reviewed request DTO when input is required and returns one operation-specific DTO. `setup_status` reports a fixed executable state and discovery source without exposing the executable path. `workspace_pick` owns the Windows native folder dialog in Rust and returns only a canonical validated directory. Workspace validation rejects missing, non-directory, oversized, and control-bearing values before use. The versioned recent-workspace store is capped at eight entries, preserves unavailable entries for explicit removal, and may remember the last selected session identifier per workspace. It stores no credentials, transcripts, or Grok session files. A preference read or write failure becomes a bounded warning without preventing an otherwise valid workspace selection or ACP session operation. `workspace_changes` is a read-only Git inspection of the selected canonical folder. It never stages, commits, discards, or deletes files, and the DTO always reports `attributableToSession: false`. Diff size, file count, and line count are bounded; binary, credential-like, escaped, and out-of-boundary paths are omitted. `runtime_diagnostics` returns bounded process health (worker liveness, consecutive failures, stderr counters, containment class) without stderr text, PIDs, or launch arguments. Runtime and preference errors become bounded application errors with a fixed code, redacted diagnostic, and recoverability flag. After the runtime is ready, the setup controller opens the first available recent workspace once. The session controller then resumes that workspace's remembered session through ACP if the identifier is still listed. A failed or disconnected runtime is recovered with `runtime_restart` or `runtime_start`; a newer event generation clears live conversation state before that resume, so stale events cannot attach to the restored session.
